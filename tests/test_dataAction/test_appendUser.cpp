...
TEST(AppendUserTest, UserToProductMap) {
    std::map<int, std::vector<int>> data = {{1, {102, 103}}, {4, {105, 106}}};
    ...
    EXPECT_EQ(data_manager->getMapUserToProducts()[5], std::vector<int>());
    delete data_manager;
}
...
TEST(AppendUserTest, UserToProductMapEmpty) {
    ...
    append_user_action->execute(data_manager);
    std::map<int, std::vector<int>>updated_data = {{2, {}}};
    EXPECT_EQ(data_manager->getMapUserToProducts()[2], updated_data[2]);
    delete data_manager;
}
