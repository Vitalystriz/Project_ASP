...
This document describes the recommendation algorithm for Exercise 1 in the Advanced Systems Programming course. The system aims to suggest relevant products to a user based on the behavior of other users with similar tastes[cite: 23, 24].

## Algorithm Overview
The recommendation is based on a specific user (`userid`) and a specific product they are interested in (`productid`). The goal is to provide up to 10 recommendations for additional products[cite: 37, 38].
...
* **Example:** If User 1 watched products {100, 101, 102, 103} and User 5 watched {100, 102, 103, 105, 108, 111}, their similarity score is **3** (for products 100, 102, and 103)[cite: 107, 111].
...
* **Sort Order:** Products are ranked primarily by their **Total Relevance** score in descending order[cite: 122].
* **Tie-breaking:** If two products have the same relevance score, they must be sorted by their **product ID in ascending order**[cite: 124].
